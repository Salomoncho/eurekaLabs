...
# DEFAULT_HTTP_4XX_LOG_LEVEL: Set the logg level for status codes from 400 to 500
DEFAULT_HTTP_4XX_LOG_LEVEL = logging.ERROR
DEFAULT_MAX_BODY_LENGTH = 2 * 1024  # log no more than 2k bytes of content
IS_DJANGO_VERSION_GTE_3_2_0 = django_version >= (3, 2, 0, "final", 0)
DEFAULT_SENSITIVE_KEYS = [
    "Authorization", "Proxy-Authorization", "csrfmiddlewaretoken", "password", "token", "password2",
]
DEFAULT_NO_LOGGING_APP_NAMES = ['admin']
DEFAULT_NO_LOGGING_VIEW_NAMES = []
SETTING_NAMES = {
    ...
    "sensitive_headers": "REQUEST_LOGGING_SENSITIVE_KEYS",
    "no_logging_app_names": "REQUEST_NO_LOGGING_APP_NAMES",
    "no_logging_view_names": "REQUEST_NO_LOGGING_VIEW_NAMES",
}
...
class Logger:
    """Class to define the logger methods(logger.log, logger.info, etc)"""
...
class LoggingMiddleware(object):
    """Class to get api logs, default django logging system don't save all api calls"""
    def __init__(self, get_response=None):
        ...
        self.sensitive_headers = getattr(settings, SETTING_NAMES["sensitive_headers"], DEFAULT_SENSITIVE_KEYS)
        self.no_logging_app_names = getattr(settings, SETTING_NAMES["no_logging_app_names"],
                                            DEFAULT_NO_LOGGING_APP_NAMES)
        self.no_logging_view_names = getattr(settings, SETTING_NAMES["no_logging_view_names"],
                                             DEFAULT_NO_LOGGING_VIEW_NAMES)
    ...
    def process_request(self, request, response=None):
        """ Function to validate if request should be logged and how"""
    ...
    def _should_log_route(self, request):
        ...
        func = view
        # no logged app names
        if (route_match.app_name in self.no_logging_app_names) or (route_match.view_name in self.no_logging_view_names):
            no_logging = True
            no_logging_msg = getattr(func, NO_LOGGING_MSG_ATTR, 'App skipped')
    ...
    def _log_request(self, request, response):
        ...
        method_path = f"[{username}] - {request.method} {request.get_full_path()} "
        ...
        if request.method == 'POST':
            req_data = self._log_request_data(request)
            method_path += req_data  # f"body: {request.body.decode('utf-8')} - POST:{json.dumps(request.POST)} "
        ...
        self.logger.log(log_level, method_path, logging_context)

    def _log_request_data(self, request):
        # use body or POST object to get the request data
        resp = ""
        try:
            body = json.loads(request.body.decode('utf-8'))
            body_data = {k: v if k not in self.sensitive_headers else "*****" for k, v in body.items()}
            resp += f"body: {body_data}"
        except:
            body = {}
        ...
        if request.method == 'POST':
            try:
                post = request.POST
                post_data = {k: v if k not in self.sensitive_headers else "*****" for k, v in post.items()}
                resp += f" - POST:{json.dumps(post_data)} "
            except:
                post = {}
            ...
            try:
                get = request.GET
                get_data = {k: v if k not in self.sensitive_headers else "*****" for k, v in get.items()}
                resp += f" - GET:{json.dumps(get_data)}"
            except:
                get = {}
    ...
    def process_response(self, request, response):
        """Function to validate if request response should be logged and how"""
        username = str(request.user)
        resp_log = f"[{username}] resp to '{request.method} {request.get_full_path()}':  {response.status_code}"
        skip_logging, because = self._should_log_route(request)
        if skip_logging:
            return response
        ...
        # add extra info
        if response.status_code in range(400, 600) or request.method == 'POST':
            try:
                data = response.data
            except:
                data = {}
            resp_log = resp_log + f" - {json.dumps(data)}"
            try:
                resp_log = resp_log + f" - at {response.renderer_context.get('view')}"
            except:
                pass
        ...
        if response.status_code in range(400, 500):
            if self.http_4xx_log_level == DEFAULT_HTTP_4XX_LOG_LEVEL:
                ...
                self.logger.log_error(self.http_4xx_log_level, resp_log, logging_context)
            else:
                self.logger.log(self.http_4xx_log_level, resp_log, logging_context)
        elif response.status_code in range(500, 600):
            self.logger.log_error(logging.WARNING, resp_log, logging_context)
        else:
